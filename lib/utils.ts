...
export const createRepliesWithNestLevel = (replies: (PostReply & { nestLevel: never })[]) : PostReply[] => {
  replies.sort((a, b) => new Date(a.creationDate).getTime() - new Date(b.creationDate).getTime());
  ...
  function addReplyWithNestLevel(reply: PostReply, nestLevel: number) {
    ...
    replies.filter(r => r.parentReplyId === reply.replyId) // Find children of this reply
  ...
  replies.filter(reply => !reply.parentReplyId) // Top-level replies have no parentReplyId
      .forEach(reply => addReplyWithNestLevel(reply, 0)); // Start nest level at 0

  return flattenedReplies;
}

export const appendNewReply = (replies: PostReply[], newReply: PostReply & { nestLevel: never }): PostReply[] => {
  const parentReplyId = newReply.parentReplyId
  const parentIndex = replies.findIndex(reply => reply.replyId === parentReplyId);
  ...
  while(insertionIndex < replies.length && replies[insertionIndex].nestLevel > parentNestLevel) {
    insertionIndex++
  }
...
export const removeReplyTree = (replies: PostReply[], deletedReplyId: string): PostReply[] => {
  ...
  function collectDescendants(id: string) {
    idsToRemove.add(id);
    replies.filter(reply => reply.parentReplyId === id) // Find direct children
